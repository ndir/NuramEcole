...
public class UtilisateurService  implements Serializable {
	...
	public void find() {
		...
		if(filtreNom != null && !filtreNom.isEmpty()) criteres.add(Restrictions.ilike("nom",filtreNom));
		if(filtrePrenom != null && !filtrePrenom.isEmpty()) criteres.add(Restrictions.ilike("prenom",filtrePrenom));
		if(profileSearch != null ) criteres.add(Restrictions.eq("profile",profileSearch));

		criteres.addOrder(Order.asc("nom"));
		criteres.addOrder(Order.asc("prenom"));

		listeUtilisateurs = criteres.list();
		//listeUtilisateurs =  this.dataSource.createQuery("select distinct u  from Utilisateur u ").list();
	}
